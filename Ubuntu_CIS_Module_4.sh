...
ensure_lockout_for_failed_password_attempts() {
    ...
    if grep -q "deny" /etc/pam.d/common-auth; then
        ...
        sed -i 's/^auth    required    pam_tally.so.*/auth    required    pam_tally.so onerr=fail deny=5 unlock_time=900/' /etc/pam.d/common-auth
...
ensure_password_reuse_limited() {
    ...
    if grep -q "remember" /etc/pam.d/common-password; then
        ...
        sed -i 's/^password    \[success=1 default=ignore\]    pam_unix.so obscure sha512/password    [success=1 default=ignore]    pam_unix.so obscure sha512 remember=5/' /etc/pam.d/common-password
...
ensure_password_hashing_algorithm() {
    echo "Ensuring password hashing algorithm is SHA-512..."
    if grep -q "sha512" /etc/pam.d/common-password; then
        echo "Password hashing algorithm is already SHA-512."
        return 0
    else
        sed -i 's/^password    \[success=1 default=ignore\]    pam_unix.so obscure sha512/password    [success=1 default=ignore]    pam_unix.so obscure sha512/' /etc/pam.d/common-password
        echo "Password hashing algorithm is now SHA-512."
    fi
}

# Call the function to ensure password hashing algorithm is SHA-512
ensure_password_hashing_algorithm
